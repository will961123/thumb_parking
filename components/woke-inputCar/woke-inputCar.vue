<template>
	<div class="wrapper">
		...
		<div ref="inputBlock" @click="open" v-if="defaultType" class="data-show"> 
			<div :class="['data-show-block', { active: showLength === n }]" v-for="n in [0,1,2,3,4,5,6]" :key="n">{{ inputValue[n] !== undefined ? inputValue[n] : '' }}</div>
		</div>
		...
		<transition name="keybordSlide">
			<div v-if="visible" class="keybord-wrap">
				<header class="keybord-header">
					...
					<span :class="[{ gray: showLength !== 7 }]" @click.stop="submit()">完成</span>
				</header>

				<div class="keybord-keys">
					<ul class="keybord-keys-word-wrap" v-if="keybordType === '字'">
						<li @click.stop="inputWord(item)" class="button" v-for="item in lineOneToThree" :key="item">{{ item }}</li>
					</ul>
					<div v-else>
						<ul class="keybord-keys-abc-wrap">
							<li @click.stop="inputWord(item)" class="button" v-for="item in abcLineOne" :key="item">{{ item }}</li>
						</ul>
						<ul class="keybord-keys-abc-wrap">
							<li @click.stop="inputWord(item)" class="button" v-for="item in abcLineTwo" :key="item">{{ item }}</li>
						</ul>
						<ul class="keybord-keys-abc-wrap-short">
							<li @click.stop="inputWord(item)" class="button" v-for="item in abcLineThree" :key="item">{{ item }}</li>
						</ul>
					</div>

					<div class="keybord-keys-bottom">
						<div @click.stop="toggle" class="big-button">{{ keybordType }}</div>
						<ul class="keybord-keys-bottom-line">
							<li @click.stop="inputWord(item)" class="button" v-for="item in judgeList" :key="item">{{ item }}</li>
						</ul>
